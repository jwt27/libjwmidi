...
namespace jw::midi
{
    using clock = jw::midi::config::clock;
    ...
    struct timed_message : untimed_message
    {
        ...
        template<typename M> requires (std::constructible_from<untimed_message, M>
                                       and not std::derived_from<std::remove_cvref_t<M>, std::istream>
                                       and not std::derived_from<std::remove_cvref_t<M>, untimed_message>)
